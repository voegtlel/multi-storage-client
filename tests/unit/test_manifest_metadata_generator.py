...
def test_file_metadata_generator() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        storage_client_config_dict = {
            "profiles": {
                "posix-data": {"storage_provider": {"type": "file", "options": {"base_path": f"{data_parent_folder}"}}},
                "posix-manifest": {"storage_provider": {"type": "file", "options": {"base_path": f"{tmpdir}"}}},
                "posix-data-with-metadata": {
                    "storage_provider": {"type": "file", "options": {"base_path": f"{data_parent_folder}"}},
                    "metadata_provider": {
                        "type": "manifest",
                        "options": {
        ...
        manifest_timestamp = datetime.fromisoformat(
            os.path.basename(os.path.dirname(manifest_main_paths[0]))
        ).isoformat(timespec="seconds")

        # Only other file besides manifest parts should be manifest_main.json.
        assert (
            len(list(manifest_storage_client.list(prefix=os.path.join(manifest_prefix, manifest_timestamp))))
            - expected_manifest_parts_count
            == 1
        )
        assert (
            len(list(manifest_storage_client.list(prefix=os.path.join(manifest_prefix, manifest_timestamp, "parts"))))
            == expected_manifest_parts_count
        )
